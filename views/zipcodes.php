...
class ZipcodesView	{
	...
	function add_javascript()	{	
		...
		<script type="text/javascript" src="<?php echo plugins_url(); ?>/educationexpress/js/jquery.form.js"></script>
		...
		jjQuery(document).ready(function(){
			jjQuery("#form_zipcode").validate({
					...
					submitHandler: function(form) {
						...
						jjQuery.post(
							wpurl+'/wp-admin/admin-ajax.php?action=zipcode_ajax', 
							jjQuery('#form_zipcode').serialize() +'&'+ jjQuery('#form_table').serialize()+'&'+jjQuery.param({ 'pagination_type': '' }),
		...
		jQuery(document).ready(function() {
			addForm = (function() {
				...
				jQuery("#form-dialog-modal").dialog('option', 'title', 'Add Zipcode ');
				jQuery("#form-dialog-modal").dialog("open");
				resetForm();
			});
			
			resetForm = (function() {
				jQuery("#zipcodeid").val('');
				jQuery("#zipcodename").val('');
				jQuery("#schools").val('');
			});
		
			editZipcode = (function(id) {
				jjQuery.blockUI({ message: '<div style="font-weight:bold" valign="top"><img src="'+plugins_url+'/educationexpress/images/loading.gif" /></div>' });
				jQuery.post(wpurl+'/wp-admin/admin-ajax.php?action=zipcode_ajax',{ zipcodeid: id, task: 'editZipcode' },function(response) {
					var data = jQuery.parseJSON(response);
					jQuery("#zipcodeid").val(id);
					jQuery("#zipcodename").val( stripslashes(data.name) );
					jQuery("#schools").val(data.school_id);
					jjQuery.unblockUI();
					
					jQuery("#taskspan").html('Edit');
					jQuery("#form-dialog-modal").dialog('option', 'title', 'Edit Zipcode ');
					jQuery("#form-dialog-modal").dialog("open");
				});
			});
			...
			trashZipcode = (function(id) {
				set_subsub('');
				jjQuery.blockUI({ message: '<div style="font-weight:bold" valign="top"><img src="'+plugins_url+'/educationexpress/images/loading.gif" /></div>' });
				jQuery.post(
					wpurl+'/wp-admin/admin-ajax.php?action=zipcode_ajax',
					jjQuery('#form_table').serialize()+'&'+jjQuery.param({ 'zipcodeid': id })+'&'+jjQuery.param({ 'task': 'trashZipcode' })+'&'+jjQuery.param({ 'pagination_type': '' }),
					function(responsedata,textStatus,jqXHR) {
						if(textStatus == 'success')	{
			...
			restoreZipcode = (function(id) {
				set_subsub('trash');
				jjQuery.blockUI({ message: '<div style="font-weight:bold" valign="top"><img src="'+plugins_url+'/educationexpress/images/loading.gif" /></div>' });
				jQuery.post(
					wpurl+'/wp-admin/admin-ajax.php?action=zipcode_ajax',
					jjQuery('#form_table').serialize()+'&'+jjQuery.param({ 'zipcodeid': id })+'&'+jjQuery.param({ 'task': 'restoreZipcode' })+'&'+jjQuery.param({ 'pagination_type': '' }),
					function(responsedata,textStatus,jqXHR) {
						if(textStatus == 'success')	{
							jQuery("#loadtable").html(responsedata);
							jjQuery.unblockUI();
			...
			removeZipcode = (function(id) {
				if(confirm("Are you sure you want to delete this record?")){
					set_subsub('trash');
					jjQuery.blockUI({ message: '<div style="font-weight:bold" valign="top"><img src="'+plugins_url+'/educationexpress/images/loading.gif" /></div>' });
					jQuery.post(
						wpurl+'/wp-admin/admin-ajax.php?action=zipcode_ajax',
						jjQuery('#form_table').serialize()+'&'+jjQuery.param({ 'zipcodeid': id })+'&'+jjQuery.param({ 'task': 'removeZipcode' })+'&'+jjQuery.param({ 'pagination_type': '' }),
						function(responsedata,textStatus,jqXHR) {
							if(textStatus == 'success')	{
								jQuery("#loadtable").html(responsedata);
			...
			get_zipcode = (function(param) {
				set_subsub(param);
				var zipcode_filter_school = jQuery("#zipcode_filter_school").val();
				jjQuery.blockUI({ message: '<div style="font-weight:bold" valign="top"><img src="'+plugins_url+'/educationexpress/images/loading.gif" /></div>' });
				jQuery.post(
					wpurl+'/wp-admin/admin-ajax.php?action=zipcode_ajax',
					jjQuery('#form_table').serialize()+'&'+jjQuery.param({ 'task': 'get_zipcode' })+'&'+jjQuery.param({ 'trashstatus': param })+'&'+jjQuery.param({ 'pagination_type': '' }),
					function(responsedata,textStatus,jqXHR) {
						if(textStatus == 'success')	{
							jQuery("#loadtable").html(responsedata);
							jjQuery.unblockUI();
			...
			uploadCSV = (function() {
				jQuery("#zipcode_school").val();
				jQuery("#taskspan").html('Upload CSV');
			...
			filterBySchool = (function() {
				...
				jQuery.post(
					wpurl+'/wp-admin/admin-ajax.php?action=zipcode_ajax',
					jjQuery('#form_table').serialize()+'&'+jjQuery.param({ 'task': 'filter_zipcode' })+'&'+jjQuery.param({ 'status': status })+'&'+jjQuery.param({ 'pagination_type': '' }),
					function(responsedata,textStatus,jqXHR) {
						if(textStatus == 'success')	{
							jQuery("#loadtable").html(responsedata);
							jjQuery.unblockUI();
			...
			pagination = (function(param) {
				...
				jQuery.post(
					wpurl+'/wp-admin/admin-ajax.php?action=zipcode_ajax',
					jjQuery('#form_table').serialize()+'&'+jjQuery.param({ 'task': 'pagination_zipcode' })+'&'+jjQuery.param({ 'trashstatus': status })+'&'+jjQuery.param({ 'pagination_type': param }),
					function(responsedata,textStatus,jqXHR) {
						if(textStatus == 'success')	{
							jQuery("#loadtable").html(responsedata);
							jjQuery.unblockUI();
	...
	function table_header($param=null)	{
		...
		<tr>
			...
			<th scope="col" width="50%" style="cursor:pointer;">Zip Code</th>
			<th scope="col" width="48%" style="cursor:pointer;">School</th>
		</tr>
		<?php
	}
	...
	function load_table($datas, $count_trash=null, $count_all=null, $currentselected=null, $filterparam=null)	{
		...
		<ul class="subsubsub">
			<li class="all"><a <?php echo $all_current; ?> style="cursor:pointer;" onclick="get_zipcode('')" id="tab_all" >All <span class="count">(<?php echo $count_all; ?>)</span></a> |</li>
			<li class="trash"><a <?php echo $trash_current; ?> style="cursor:pointer;" onclick="get_zipcode('trash')" id="tab_trash">Trash <span class="count">(<?php echo $count_trash; ?>)</span></a></li>
		</ul>
		
		<p class="search-box">
			<input type="search" value="<?php if(isset($_POST['textsearch']))	{	echo trim($_POST['textsearch']);	}	?>" name="textsearch" id="textsearch" />
			<input type="button" value="Search Zip Code" class="button" onclick="filterBySchool()" />
		</p>
			...
			<div class="alignleft actions">
				<select name="bulkaction" id="bulkaction">
					...
					<?php if($currentselected == 'trash')	{ ?>
						<option value="restoreZipcodes">Restore</option>
						<option value="removeZipcodes">Delete Permanently</option>
					<?php } else	{ ?>
						<option value="trashZipcodes">Move to Trash</option>
					<?php } ?>
				</select>
				<input type="button" value="Apply" class="button action" id="doaction" name="doaction" onclick="bulkApply()" />
				<?php if($currentselected != 'trash')	{ ?>
					<input type="button" value="Upload CSV" class="button action" id="doaction" name="doaction" onclick="uploadCSV()" />
				<?php } ?>
			</div>
			<div class="alignleft actions">
				<?php 
				if(isset($_POST['zipcode_filter_school']))	
					$selected_filter = $_POST['zipcode_filter_school'];
				else
					$selected_filter = '';
				FormHelper::dropDownObj('schools',1,'',$selected_filter,'zipcode_filter_school', 'View all schools'); 
				?>
				<input type="button" value="Filter" class="button action" id="doaction" name="doaction" onclick="filterBySchool()" />
			</div>
		...
		<table cellspacing="0" class="widefat post fixed">
			<thead><?php ZipcodesView::table_header(1); ?></thead>
			<tfoot><?php ZipcodesView::table_header(); ?></tfoot>
			<tbody>
				<?php
				if(count($datas) > 0)	{
					$i=0;
						...
						<tr>
							...
							<?php if($data->trashed == 0)	{	?>
								...
								<div class="row-actions-visible">
									<span><a style="cursor:pointer" onclick="editZipcode('<?php echo $data->id; ?>')">Edit</a> | </span>
									<span><a style="cursor:pointer" onclick="trashZipcode('<?php echo $data->id; ?>')" >Trash</a> </span>
								</div>
							<?php } else	{	?>	
								<strong><?php echo stripslashes($data->name); ?></strong>
								<div class="row-actions-visible">
									<span><a style="cursor:pointer" onclick="restoreZipcode('<?php echo $data->id; ?>')">Restore</a> | </span>
									<span><a style="cursor:pointer" onclick="removeZipcode('<?php echo $data->id; ?>')" >Delete Permanently </a></span>
								</div>
							<?php } 	?>	
							</td>
				...
				else	{ ?> <tr class="no-items"><td colspan="3" class="colspanchange">No records found.</td></tr> <?php	} 	?>
			</tbody>
		</table>
		</form>
		<?php
	...
	function load_zipcodes($datas, $count_trash=null, $count_all=null, $currentselected=null, $filterparam=null)	{
		?>
		<div class="wrap">
			<div class="icon32" id="icon-plugins"><br></div>
			<h2>Zip Codes <a class="add-new-h2" onclick="addForm()" style="cursor:pointer;">Add New</a> </h2>

			<!--<div class="updated below-h2" id="message"><p>Item permanently deleted.</p></div>-->

			<div id="resultmessage"></div>
			<div id="loadtable"><?php ZipcodesView::load_table($datas, $count_trash, $count_all, $currentselected, $filterparam); ?></div>
			<br/>
			
			<div id="load_form">
				<?php ZipcodesView::get_form(); ?>
				<?php ZipcodesView::csv_form(); ?>
			</div>
		...
		<div id="form-dialog-modal" title="Add">
			...
			<form id="form_zipcode" action="" method="post">
				...
				<tr>
					<td>Zipcode</td>
					<td>
						<input type="hidden" value="" name="zipcodeid" id="zipcodeid"  />
				...
				<tr>
					<td>&nbsp;</td>
					<td>
						<input type="hidden" value="savezipcode" name="task" id="task" />
	...
	function csv_form()	{
		...
		<div id="formcsv-dialog-modal" title="Upload">
			...
			<form id="form_csv" action="<?php bloginfo('wpurl'); ?>/wp-admin/admin-ajax.php?action=zipcode_ajax" method="post" enctype="multipart/form-data">
				<table><tbody>
				<tr>
					<td>School</td>
					<td><?php FormHelper::dropDownObj('schools',1,'','','zipcode_school'); ?></td>
				</tr>
				<tr>
					<td>CSV</td>
				...
				<tr>
					<td>&nbsp;</td>
					<td>
						<input type="hidden" value="uploadcsv" name="task" id="task" />
						<input type="submit" value="Save" class="button-primary" />
					</td>
				</tr>
				</tbody></table>
