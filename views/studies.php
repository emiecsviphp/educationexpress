...
class StudiesView	{
	...
	function add_javascript()	{	
		...
		jjQuery(document).ready(function(){
			jjQuery("#form_study").validate({
					...
					submitHandler: function(form) {
						...
						jjQuery.post(
							wpurl+'/wp-admin/admin-ajax.php?action=study_ajax', 
							jjQuery('#form_study').serialize() +'&'+ jjQuery('#form_table').serialize()+'&'+jjQuery.param({ 'pagination_type': '' }),
		...
		jQuery(document).ready(function() {
			addForm = (function() {
				...
				jQuery("#form-dialog-modal").dialog('option', 'title', 'Add Study ');
			...
			resetForm = (function() {
				jQuery("#studyid").val('');
				jQuery("#studyname").val('');
			});
		
			editStudy = (function(id) {
				jjQuery.blockUI({ message: '<div style="font-weight:bold" valign="top"><img src="'+plugins_url+'/educationexpress/images/loading.gif" /></div>' });
				jQuery.post(wpurl+'/wp-admin/admin-ajax.php?action=study_ajax',{ studyid: id, task: 'editStudy' },function(response) {
					var data = jQuery.parseJSON(response);
					jQuery("#studyid").val(id);
					jQuery("#studyname").val( stripslashes(data.name) );
					
					jjQuery.unblockUI();
					
					jQuery("#taskspan").html('Edit');
					jQuery("#form-dialog-modal").dialog('option', 'title', 'Edit Study ');
					jQuery("#form-dialog-modal").dialog("open");
				});
			});
			...
			trashStudy = (function(id) {
				set_subsub('');
				jjQuery.blockUI({ message: '<div style="font-weight:bold" valign="top"><img src="'+plugins_url+'/educationexpress/images/loading.gif" /></div>' });
				jQuery.post(
					wpurl+'/wp-admin/admin-ajax.php?action=study_ajax',
					jjQuery('#form_table').serialize()+'&'+jjQuery.param({ 'task': 'trashStudy' })+'&'+jjQuery.param({ 'studyid': id })+'&'+jjQuery.param({ 'pagination_type': '' }),
					function(responsedata,textStatus,jqXHR) {
					if(textStatus == 'success')	{
			...
			restoreStudy = (function(id) {
				set_subsub('trash');
				jjQuery.blockUI({ message: '<div style="font-weight:bold" valign="top"><img src="'+plugins_url+'/educationexpress/images/loading.gif" /></div>' });
				jQuery.post(
					wpurl+'/wp-admin/admin-ajax.php?action=study_ajax',
					jjQuery('#form_table').serialize()+'&'+jjQuery.param({ 'task': 'restoreStudy' })+'&'+jjQuery.param({ 'studyid': id })+'&'+jjQuery.param({ 'pagination_type': '' }),
					function(responsedata,textStatus,jqXHR) {
						if(textStatus == 'success')	{
							jQuery("#loadtable").html(responsedata);
							jjQuery.unblockUI();
			...
			removeStudy = (function(id) {
				if(confirm("Are you sure you want to delete this record?")){
					...
					jQuery.post(
						wpurl+'/wp-admin/admin-ajax.php?action=study_ajax',
						jjQuery('#form_table').serialize()+'&'+jjQuery.param({ 'task': 'removeStudy' })+'&'+jjQuery.param({ 'studyid': id })+'&'+jjQuery.param({ 'pagination_type': '' }),
						function(responsedata,textStatus,jqXHR) {
							if(textStatus == 'success')	{
								jQuery("#loadtable").html(responsedata);
			...
			get_study = (function(param) {
				set_subsub(param);
				jjQuery.blockUI({ message: '<div style="font-weight:bold" valign="top"><img src="'+plugins_url+'/educationexpress/images/loading.gif" /></div>' });
				jQuery.post(
					wpurl+'/wp-admin/admin-ajax.php?action=study_ajax',
					jjQuery('#form_table').serialize()+'&'+jjQuery.param({ 'task': 'get_study' })+'&'+jjQuery.param({ 'trashstatus': param })+'&'+jjQuery.param({ 'pagination_type': '' }),
					function(responsedata,textStatus,jqXHR) {
						if(textStatus == 'success')	{
							jQuery("#loadtable").html(responsedata);
							jjQuery.unblockUI();
			...
			filterStudies = (function() {
				...
				jQuery.post(
					wpurl+'/wp-admin/admin-ajax.php?action=study_ajax',
					jjQuery('#form_table').serialize()+'&'+jjQuery.param({ 'task': 'filterStudies' })+'&'+jjQuery.param({ 'status': status })+'&'+jjQuery.param({ 'pagination_type': '' }),
					function(responsedata,textStatus,jqXHR) {
						if(textStatus == 'success')	{
							jQuery("#loadtable").html(responsedata);
							jjQuery.unblockUI();
			...
			pagination = (function(param) {
				...
				jQuery.post(
					wpurl+'/wp-admin/admin-ajax.php?action=study_ajax',
					jjQuery('#form_table').serialize()+'&'+jjQuery.param({ 'task': 'pagination_study' })+'&'+jjQuery.param({ 'trashstatus': status })+'&'+jjQuery.param({ 'pagination_type': param }),
					function(responsedata,textStatus,jqXHR) {
						if(textStatus == 'success')	{
							jQuery("#loadtable").html(responsedata);
							jjQuery.unblockUI();
	...
	function load_table($datas, $count_trash=null, $count_all=null, $currentselected=null, $filterparam=null)	{
		...
		<ul class="subsubsub">
			<li class="all"><a <?php echo $all_current; ?> style="cursor:pointer;" onclick="get_study('')" id="tab_all" >All <span class="count">(<?php echo $count_all; ?>)</span></a> |</li>
			<li class="trash"><a <?php echo $trash_current; ?> style="cursor:pointer;" onclick="get_study('trash')" id="tab_trash">Trash <span class="count">(<?php echo $count_trash; ?>)</span></a></li>
		</ul>
		
		<p class="search-box">
			<input type="search" value="<?php if(isset($_POST['textsearch']))	{	echo trim($_POST['textsearch']);	}	?>" name="textsearch" id="textsearch" />
			<input type="button" value="Search Studies" class="button" onclick="filterStudies()" />
		</p>
			...
			<div class="alignleft actions">
				<select name="bulkaction" id="bulkaction">
					...
					<?php if($currentselected == 'trash')	{ ?>
						<option value="restoreStudies">Restore</option>
						<option value="removeStudies">Delete Permanently</option>
					<?php } else	{ ?>
						<option value="trashStudies">Move to Trash</option>
					<?php } ?>
				</select>
		...
		<table cellspacing="0" class="widefat post fixed">
			<thead><?php StudiesView::table_header(1); ?></thead>
			<tfoot><?php StudiesView::table_header(); ?></tfoot>
			<tbody>
				<?php
				if(count($datas) > 0)	{
					$i=0;
						...
						<tr>
							...
							<?php if($data->trashed == 0)	{	?>
								...
								<div class="row-actions-visible">
									<span><a style="cursor:pointer" onclick="editStudy('<?php echo $data->id; ?>')">Edit</a> | </span>
									<span><a style="cursor:pointer" onclick="trashStudy('<?php echo $data->id; ?>')" >Trash</a> </span>
								</div>
							<?php } else	{	?>	
								<strong><?php echo stripslashes($data->name); ?></strong>
								<div class="row-actions-visible">
									<span><a style="cursor:pointer" onclick="restoreStudy('<?php echo $data->id; ?>')">Restore</a> | </span>
									<span><a style="cursor:pointer" onclick="removeStudy('<?php echo $data->id; ?>')" >Delete Permanently </a></span>
								</div>
							<?php } 	?>	
							</td>
						</tr>
	...
	function load_studies($datas, $count_trash=null, $count_all=null, $currentselected=null, $filterparam=null)	{
		?>
		<div class="wrap">
			<div class="icon32" id="icon-plugins"><br></div>
			<h2>Studies <a class="add-new-h2" onclick="addForm()" style="cursor:pointer;">Add New</a> </h2>

			<!--<div class="updated below-h2" id="message"><p>Item permanently deleted.</p></div>-->

			<div id="resultmessage"></div>
			<div id="loadtable"><?php StudiesView::load_table($datas, $count_trash, $count_all, $currentselected, $filterparam); ?></div>
			<br/>
			
			<div id="load_form">
				<?php StudiesView::get_form(); ?>
			</div>
	...
	function get_form()	{
		...
		<div id="form-dialog-modal" title="Add">
			<p> <span id="taskspan"> Add </span> the name of the study</p>
			
			<form id="form_study" action="" method="post">
				<table><tbody>
				<tr>
					<td>Name</td>
					<td>
						<input type="hidden" value="" name="studyid" id="studyid"  />
						<input type="text" name="studyname" id="studyname" style="width:300px;" />
					</td>
				</tr>
				...
				<tr>
					<td>&nbsp;</td>
					<td>
						<input type="hidden" value="savestudy" name="task" id="task" />
